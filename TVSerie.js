...
document.querySelector('.title-h1').innerHTML=`${data.name}`
vote_average=String(data.vote_average).slice(0, 3)
...
data.seasons.forEach(element=>{
    let GenreSelect=document.querySelector('.seasons');
...
document.querySelector('.seasons').addEventListener('change',(e)=>{
    let body=document.querySelector('body');
    body.style.background=`url('${sea}')`
    console.log(GenreID)
